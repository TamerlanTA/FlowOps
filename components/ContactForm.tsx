  ...
  return (
    <label
      ...
      className={`pointer-events-none absolute left-4 z-10 transition-all duration-200 ${
        active ? "top-2 text-xs font-medium text-[#00d4c0]" : "top-4 text-sm text-slate-400"
      }`}
    >
      {label}
      {required ? <span className="ml-0.5 text-[#00d4c0]">*</span> : null}
    </label>
  );
}
...
export default function ContactForm() {
  ...
  const inputClasses =
    "w-full rounded-xl border border-white/10 bg-white/5 px-4 pb-3 pt-7 text-sm text-white outline-none transition-all duration-300 focus:border-[rgba(0,212,192,0.4)] focus:bg-white/10 focus:shadow-[0_0_0_3px_rgba(0,212,192,0.1)] sm:rounded-2xl min-h-[52px]";
          ...
          <Reveal variant="right" delay={150}>
            ...
            >
              ...
              <button
                type="submit"
                disabled={formDisabled}
                className="inline-flex w-full items-center justify-center rounded-xl btn-primary px-6 py-3 text-sm font-bold text-white transition-all duration-300 disabled:cursor-not-allowed disabled:opacity-70 active:scale-[0.98] sm:rounded-2xl md:hover:scale-[1.02]"
              >
                {isSubmitting ? (
                  <>
                    <svg
