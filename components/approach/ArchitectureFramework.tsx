          ...
          <div className="absolute bottom-0 left-1/2 top-0 hidden w-px -translate-x-1/2 lg:block">
            <Reveal variant="fade" delay={200}>
              <div
                ...
                style={{
                  background:
                    "linear-gradient(to bottom, transparent, rgba(0,212,192,0.18) 10%, rgba(0,212,192,0.18) 90%, transparent)",
                }}
              />
            </Reveal>
          </div>
              ...
              >
                ...
                >
                  <div className={`flex-1 ${index % 2 === 0 ? "lg:pr-16 lg:text-right" : "lg:pl-16 lg:text-left"}`}>
                    <article className="group rounded-3xl p-8 transition-all duration-500 liquid-glass prismatic-edge md:hover:-translate-y-1 md:hover:shadow-[0_12px_40px_rgba(0,0,0,0.3)]">
                      <div className={`mb-4 flex items-center gap-4 ${index % 2 === 0 ? "lg:flex-row-reverse" : ""}`}>
                        <div className="flex size-12 shrink-0 items-center justify-center rounded-2xl text-[#00d4c0] transition-transform duration-500 liquid-glass-subtle md:group-hover:scale-110">
                          <layer.icon className="size-5" />
                        </div>
                        <div>
                          <span className="text-xs font-semibold uppercase tracking-widest text-[rgba(0,212,192,0.7)]">
                            Layer {layer.number}
                          </span>
                          <h3 className="text-xl font-bold text-white">{layer.label}</h3>
                        </div>
                      </div>
                      <p className="mb-2 text-sm font-medium text-[rgba(0,212,192,0.8)]">{layer.description}</p>
                  ...
                  <div className="relative z-10 hidden shrink-0 lg:flex">
                    <div
                      className="flex size-14 items-center justify-center rounded-full border border-[rgba(0,212,192,0.2)] liquid-glass-elevated"
                      style={{ boxShadow: `0 0 24px ${layer.color}` }}
                    >
                      <span className="text-sm font-bold text-[#00d4c0]">{layer.number}</span>
                    </div>
                  </div>
